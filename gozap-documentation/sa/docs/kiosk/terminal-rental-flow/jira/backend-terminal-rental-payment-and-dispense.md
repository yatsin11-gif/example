...
2. Команда станции на выдачу PowerBank отправляется только после подтверждённой успешной оплаты.
3. После успешной оплаты пользователь получает SMS о создании аккаунта/начале аренды через Twilio.
4. Отклонённая оплата не приводит к выдаче PowerBank.
