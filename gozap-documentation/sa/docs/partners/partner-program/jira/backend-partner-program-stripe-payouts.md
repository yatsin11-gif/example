...
- отложенная выплата при отсутствии счёта/карты на момент планового запуска: накопленный баланс выплачивается сразу после того, как партнёр указывает счёт/карту через Stripe-редирект, далее выплаты продолжаются по обычному ежемесячному расписанию
...
5. Если на момент планового запуска у партнёра не указан банковский счёт/карта, выплата откладывается без потери баланса; как только партнёр указывает счёт/карту через Stripe, накопленный баланс выплачивается немедленно, и далее выплаты продолжаются по обычному ежемесячному расписанию.
6. Партнёр может перейти (редирект) в Stripe для просмотра истории начислений и истории выплат.
7. Партнёр может выгрузить CSV по заработку и выплатам за выбранный период.
